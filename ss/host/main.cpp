...
int main()
{
	...
	pos_t * input = new pos_t[dim*(dim-1)/2];
	int k=0;
	for(int i = 0; i<dim; i++)
	{	
	...
	cout<<"data size is: "<<sizeof(float)*dim*dim/1024<<"KB"<<endl;

	std::cout<<"Data loaded..."<<std::endl;
	Scheduler scheduler((void *)matdata, sizeof(float)*dim*dim, input, dim*(dim-1)/2, sizeof(pos_t));
