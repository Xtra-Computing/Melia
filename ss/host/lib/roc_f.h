...
unsigned short key_size, void *value, unsigned short value_size)
{
	...
	int lock_status;
       ...
       { 
         ...
         uint4 key_data_unit4 =( (object->align_bucket)[index]).bucket_data_int4; /*load the unt4 data for the merge*/
         ...
         if (key_data_unit4.y == 0)//if((object->pairs_per_bucket)[index]==0)
          {	
             ...
             ( (object->align_bucket)[index]).bucket_data_int4 =  key_data_unit4; 			 
/*					((object->buckets)[index]).x = k;
					((object->buckets)[index]).y = v;
...
*/
           ...
           else
            {
               ...
               if(equal_global_and_global(key, key_size, key_data, size))
               {
                    unsigned short value_ro_size = 4; //cget_value_size(object, index);
