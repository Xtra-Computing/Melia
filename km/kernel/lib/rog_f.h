...
void ginit(__global Reduction_Object_G *object)
{
 ...
 //   int local_size    =get_local_size(0);
	...
	if(local_id == 0)
	{
		(object->memory_offset)[block_id] = (block_id * MAX_POOL_G)/block_size;
	}	
}
       ...
       while(DoWork)
        {
           ...
           while (lock_status == 0)
            { 
             ...
             uint4 key_data_unit4 = *(__global uint4 *) (& (object->align_bucket)[index]); /*load the unt4 data for the merge*/
...
*/
               //  key_data_unit4.xyzw = (uint4) ( (key_size<<16)|(value_size&(0xffff)), 1, k, v );
		key_data_unit4.xyzw = (uint4) ( (value_size<<16)|(key_size&(0xffff)), 1, k, v );

                 *(__global uint4 *) (& (object->align_bucket)[index]) =  key_data_unit4; 		
			 
                 __global char *key_data_start = (global char *)gget_address(object, k);
                 __global void *value_data_start = gget_address(object, v);

               *( ( __global int *)key_data_start) = *( ( __local int *)key); //copyVal_local_to_global(key_data_start, key, key_size);
             ...
             else
              {
                unsigned int size              = (key_data_unit4.x)&(0xffff);//(key_data_unit4.x)>>16; //cget_key_size(object, index);
                 __global void *key_data = gget_address(object, (key_data_unit4.z) )  ;//cget_key_address(object, index);

					/*if equal, conduct reduce*/
               if(equal_local_and_global(key, key_size, key_data, size)) //(equal_global_and_global(key, key_size, key_data, size))
               {
                    unsigned short value_ro_size = (key_data_unit4.y) >>16; //4; //  cget_value_size(object, index);
                     __global void *value_ro_data = gget_address(object, (key_data_unit4.w) )  ;//cget_value_address(object, index);
