...
//	int block_id = global_id/CPU_LOCAL_THREADS;
	...
	unsigned int offset = atomic_add(&(roc->memory_offset)[0], align_size ); //size
...
//	unsigned int h = hash_global(key, key_size);
//	unsigned int index = h%NUM_BUCKETS_C;
//	printf("object->pairs_per_bucket)[%d] %d \n",index, (object->pairs_per_bucket)[index]);
//	while(!finish)
//	{
//		DoWork = 1;
	int lock_status = 0;
